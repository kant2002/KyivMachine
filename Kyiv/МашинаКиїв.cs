...
public class МашинаКиїв
{
    ...
    public List<string> НадрукованіРядки { get; } = new();
    private bool МагнітнаГоловкаМожеПисати;
    private int ВибранийБарабан;
    private int ПозиціяМагнітноїГоловки;
    ...
    public МашинаКиїв()
    {
        ...
        ПассивноеЗапоминающееУстройство[Із8РічноїАдреси(121)] = ПарсерКоманд.Сконвертувати("01 0005 3011 0005");
        ПассивноеЗапоминающееУстройство[Із8РічноїАдреси(122)] = ПарсерКоманд.Сконвертувати("05 0000 0000 3117");
        ПассивноеЗапоминающееУстройство[Із8РічноїАдреси(123)] = ПарсерКоманд.Сконвертувати("12 3011 0005 0004");
    ...
    public ulong ПрочитатиПамять(int адреса)
    {
        ...
        if (адреса < 2047)
        {
            ...
            return ПассивноеЗапоминающееУстройство[адреса - 1536];
        }
    ...
    public void ВиконатиКоманду(СтруктураКоманди команда)
    {
        switch (команда.Код)
        {
            ...
            case КодОперації.СложениеКоманд:
                {
                    var значення1 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса1));
                    var значення2 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса2));
                    var значення = ДодаванняКоманд(значення1, значення2);
                    ЗаписатиПамять(ВзятиВідноснуАдреса(команда.Адреса3), значення);
                    РегістрС += 1;
                    РегістрК = ПрочитатиПамять(РегістрС);
                }
            ...
            case КодОперації.СравнениеСУчетомЗнаков:
                {
                    var значення1 = РозширитиЧисло(ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса1)));
                    var значення2 = РозширитиЧисло(ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса2)));
            ...
            case КодОперації.ЦиклическоеСложение:
                {
                    var значення1 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса1));
                    var значення2 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса2));
                    var значення = (значення1 + значення2) & ~SignFlag;
                    ЗаписатиПамять(ВзятиВідноснуАдреса(команда.Адреса3), значення);
                    РегістрС += 1;
                    РегістрК = ПрочитатиПамять(РегістрС);
                }
            ...
            case КодОперації.ЛогическийСдвиг:
                {
                    var значення1 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса1));
                    var значення2 = РозширитиЧисло(ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса2)));
                    var здвиг = (int)(значення2 & 0xFFF);
                    var значення = значення2 < 0 ? (значення1 >> здвиг) : (значення1 << здвиг);
                    ЗаписатиПамять(ВзятиВідноснуАдреса(команда.Адреса3), значення);
                    РегістрС += 1;
                    РегістрК = ПрочитатиПамять(РегістрС);
                }
            ...
            case КодОперації.ОперацияРавнозначно:
                {
                    var значення1 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса1));
                    var значення2 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса2));
                    var значення = ~(значення1 ^ значення2) & (ulong.MaxValue >> 23);
                    ЗаписатиПамять(ВзятиВідноснуАдреса(команда.Адреса3), значення);
                    РегістрС += 1;
                    РегістрК = ПрочитатиПамять(РегістрС);
                }
            ...
            case КодОперації.ВыводКодов:
                {
                    var барабан = МагнітніБарабани[ВибранийБарабан];
                    for (var i = команда.Адреса1; i <= команда.Адреса2; i++)
                    {
                        var к = ПрочитатиПамять(i);
                        НадрукуватиКод(к);
                    }
            ...
            case КодОперації.ЗаписьНаМагнитныйБарабанКодовИзЯчеекЗапоминающегоУстройств:
                {
                    if (МагнітнаГоловкаМожеПисати)
                    {
                        throw new InvalidOperationException("Магнітна головка повинна бути ініційована на запис");
                    }

                    var барабан = МагнітніБарабани[ВибранийБарабан];
                    for (var i = команда.Адреса1; i <= команда.Адреса2; i++, ПозиціяМагнітноїГоловки++)
                    {
                        барабан[ПозиціяМагнітноїГоловки] = ПрочитатиПамять(i);
                    }
            ...
            case КодОперації.ЧтениеКодовСМагнитногоБарабанаВЯчейкиЗапоминающегоУстройства:
                {
                    if (!МагнітнаГоловкаМожеПисати)
                    {
                        throw new InvalidOperationException("Магнітна головка повинна бути ініційована на читання");
                    }

                    var барабан = МагнітніБарабани[ВибранийБарабан];
                    for (var i = команда.Адреса1; i<= команда.Адреса2; i++, ПозиціяМагнітноїГоловки++)
                    {
                        ЗаписатиПамять(i, барабан[ПозиціяМагнітноїГоловки]);
                    }
            ...
            case КодОперації.ОбращениеКМагнитномуБарабану:
                {
                    МагнітнаГоловкаМожеПисати = команда.Адреса1 == 1;
                    ВибранийБарабан = команда.Адреса2;
                    ПозиціяМагнітноїГоловки = команда.Адреса3;
                    РегістрС += 1;
                    РегістрК = ПрочитатиПамять(РегістрС);
                }
                break;
            ...
            case КодОперації.УсловныйПереходНаПодпрограмму:
                {
                    ...
                    if (значення1 <= 0)
                    {
                        РегістрР = (ushort)ВзятиВідноснуАдреса(команда.Адреса2);
                        РегістрС = (ushort)ВзятиВідноснуАдреса(команда.Адреса3);
                    }
                    else
                    {
            ...
            case КодОперації.УсловныйПереходПоЗнакуЧисла:
                {
                    ...
                    {
                        РегістрС = (ushort)ВзятиВідноснуАдреса(команда.Адреса2);
                    }
            ...
            case КодОперації.ПереходПоФиксатору:
                {
                    var значення1 = ПрочитатиПамять(ВзятиВідноснуАдреса(команда.Адреса1));
                    РегістрА = (ushort)значення1;
                    ЗаписатиПамять(ВзятиВідноснуАдреса(команда.Адреса3), ПрочитатиПамять(РегістрА));
    ...
    private ulong ДодаванняКоманд(ulong значення1, ulong значення2)
    {
        var знак2 = ЄЗнак(значення2);
        var число = Math.Abs(РозширитиЧисло(значення1) + РозширитиЧисло(Модуль(значення2)));
        if (знак2)
        {
            return ЗвузитиЧисло(-число);
        }
    ...
    public void НадрукуватиКод(ulong значення)
    {
        НадрукованіРядки.Insert(0, ПарсерКоманд.Сконвертувати(значення));
        if (НадрукованіРядки.Count > 100)
        {
            НадрукованіРядки.RemoveAt(100);
        }
    }
