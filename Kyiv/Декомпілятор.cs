...
public class Декомпілятор
{
    ...
    private static Лістінг СпроститиКод(Лістінг лістінг)
    {
        ...
        foreach (var блокКоду in лістінг.БлокиКоду)
        {
            var новийБлокКоду = new БлокКоду(блокКоду.ПочатковаАдреса);
            ...
            for (int i = 0; i < блокКоду.Операції.Count; i++)
            {
                ...
                if (i >= 2)
                {
                    if (операція is УмовнаОперація умовнаОперація && ((БінарнаОперація)умовнаОперація.Умова).Вираз1 is Регістр { регістр: ТипРегістра.Ц } && ((БінарнаОперація)умовнаОперація.Умова).Вираз2 is Регістр { регістр: ТипРегістра.А })
                    {
                        if (блокКоду.Операції[i - 1] is ЗберіганняЗмінної зберіганняЗмінної1
                            && зберіганняЗмінної1.МісцеЗберігання is Регістр { регістр: ТипРегістра.А }
                            && блокКоду.Операції[i - 2] is ЗберіганняЗмінної зберіганняЗмінної2
                            && зберіганняЗмінної1.Що is Константа константа1
                            && зберіганняЗмінної2.Що is Константа константа2
                            && константа1.значення != константа2.значення)
                        {
                            continue;
                        }
                    }
