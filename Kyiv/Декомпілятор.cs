...
public class Декомпілятор
{
    ...
    public static string Декомпілювати(Лістінг лістінг)
    {
        ...
        foreach (var (лінія, адреса) in лістінг.ОдиниціКоду.ВибратиБагато(ок => ок.Команди.Вибрати((к, а) => (к, ок.ПочатковаАдреса + а))))
        {
            try
            {
                if (точкиПереходу.Contains(адреса))
                {
                    писар.Відступ--;
                    писар.WriteLine($"lbl_{До8РічноїАдреси(адреса)}:");
                    писар.Відступ++;
                }

                var операції = ДоАссемблера(ПарсерКоманд.Розібрати(лінія), true)
                        .Вибрати(ВставитиЗмінні);
                foreach (var операція in операції)
                {
            ...
            {
                писар.WriteLine("// Невідома команда " + ПарсерКоманд.Сконвертувати(лінія));
            }
        }
